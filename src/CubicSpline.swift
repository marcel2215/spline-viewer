...
struct CubicSpline {
    ...
    func interpolatedValue(at x: Double) -> Double {
        guard points.count > 1 else { return points.first?.y ?? 0.0 }
        ...
        for i in 0..<n {
            h[i] = xCoords[i + 1] - xCoords[i]
            alpha[i] = i == 0 ? 0 : (3 / h[i]) * (a[i + 1] - a[i]) - (3 / h[i - 1]) * (a[i] - a[i - 1])
        }

        for i in 1..<n {
            l[i] = 2 * (xCoords[i + 1] - xCoords[i - 1]) - h[i - 1] * mu[i - 1]
            mu[i] = h[i] / l[i]
            z[i] = (alpha[i] - h[i - 1] * z[i - 1]) / l[i]
        }
    ...
    func firstDerivative(at x: Double) -> Double {
    ...
    func secondDerivative(at x: Double) -> Double {
        guard points.count > 1 else { return 0.0 }
